...
func day18(input string) (p1Result, p2Result int) {
	lines := Lines(input)
	simple := func(ex string) int {
	...
	complex := func(ex string) int {
		for {
			...
			ans := Atoi(match[1]) + Atoi(match[2])
			ex = strings.ReplaceAll(ex, match[0], fmt.Sprint(ans))
		}
		return simple(ex)
	}
...
var _ = register(18, day18, `(2 + 6) * 2 + 2 + 4

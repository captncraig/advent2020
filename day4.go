	...
	for _, p := range passports {
		...
		if valid {
			...
			for _, part := range parts {
				...
				if len(segs) != 2 {
					log.Printf("'%s' '%s'", p, part)
				}
				k, v := segs[0], segs[1]
				switch k {
				case "byr":
					d, err := strconv.Atoi(v)
					if err != nil || d < 1920 || d > 2002 {
						valid2 = false
					}
				case "iyr":
					d, err := strconv.Atoi(v)
					if err != nil || d < 2010 || d > 2020 {
						valid2 = false
					}
				case "eyr":
					d, err := strconv.Atoi(v)
					if err != nil || d < 2020 || d > 2030 {
						valid2 = false
					}
				case "hgt":
					if strings.HasSuffix(v, "in") {
						v2 := strings.TrimSuffix(v, "in")
						d, err := strconv.Atoi(v2)
						if err != nil || d < 59 || d > 76 {
							valid2 = false
						}
					} else if strings.HasSuffix(v, "cm") {
						v2 := strings.TrimSuffix(v, "cm")
						d, err := strconv.Atoi(v2)
						if err != nil || d < 150 || d > 193 {
							valid2 = false
						}
					} else {
						valid2 = false
				...
				case "ecl":
					if v != "amb" && v != "gry" && v != "blu" && v != "brn" && v != "grn" && v != "hzl" && v != "oth" {
						valid2 = false
					}
				case "pid":

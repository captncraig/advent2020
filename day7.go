...
func day7(input string) (p1Result, p2Result int) {
	lines := Lines(input)
	...
	for _, l := range lines {
		...
		outer := strings.TrimSpace(parts[0])
		inner := strings.TrimSpace(parts[1])
		contents[outer] = inner
	}
	possible := map[string]bool{}
	toSearch := []string{"shiny gold"}
	search := func(scan string) {
		for k, v := range contents {
			if !possible[k] && strings.Contains(v, scan) {
	...
	p2Result = descend("shiny gold")

	return
}

var _ = register(7, day7, `posh teal bags contain 2 faded coral bags, 3 striped crimson bags, 1 faded red bag.
